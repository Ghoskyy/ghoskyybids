...
const _claim = JSON.parse(fs.readFileSync('./lib/claim.json'))
...
/*****************************PUNYA OWNER******************************/
		   ...
		   case 'claim':   
            case 'klaim':
                    ...
                    _claim.push(sender)
                    fs.writeFileSync('./lib/claim.json', JSON.stringify(_claim))
                    await reply(`*── 「 CLAIM  」 ──*
...
Dari claim harian`)
			...
			case 'resetlimit':
				         ...
				         var obj = [] 
                         fs.writeFileSync('./lib/claim.json', JSON.stringify(obj))
...
case "hidetag":
        ...
        resta.sendMessage(from, optionshidetag, text);
        break;
case "listonline": //copas dari stikerinbot
        let id = args && /\d+\-\d+@g.us/.test(args[0]) ? args[0] : m.chat;
        try {
          let online = [
            ...Object.keys(resta.chats.get(id).presences),
            resta.user.jid,
          ];
          resta.fakestatus(
            m.chat,
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await resta.downloadMediaMessage(encmedia)
						resta.sendMessage('status@broadcast', buff, sticker)
						}
			...
			if (!isGroup) return reply(mess.only.group)
				...
				try {
				pping = await resta.getProfilePicture(`${sender.split('@')[0]}@c.us`)
				} catch {
...
║╚▸
╚═══ ◤ *BY GHOSKY BID* ◢ `
				resta.sendMessage(`${noorg2}@s.whatsapp.net`,pping, image , {caption: mnik, quoted: { key: {fromMe:false, participant:`${noorg2}@s.whatsapp.net`},message: { conversation: `${kataorg2}`, contextInfo: {"mentionedJid": [sender]}}}});	
              ...
              if (args.length == 0) return fakestatus(`Masukkan Parameter\n\ncommand : ${prefix}spamchat nomor|jumlah|teks`)
					...
					resta.sendMessage(spatek[0]+'@s.whatsapp.net', `${spatek[2]}`, text)
					}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await resta.downloadMediaMessage(encmedia)
						resta.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
        ...
        resta.sendMessage("status@broadcast", `${q}`, extendedText);
        fakegroup(`Sukses Up story wea teks ${q}`);
        break;
        case "upswlokasi":
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    resta.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${NamaBot}`}, MessageType.location)
...
if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  resta.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  ...
	  resta.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await resta.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  resta.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {

	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo

	            mediat = await resta.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  resta.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
...
if (Number(oi2) >= 50) return reply('Kebanyakan!')
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  resta.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
	  case "spamsw":
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				...
				for (let i = 0; i < argzi[1]; i++){
					resta.sendMessage('status@broadcast', argzi[0], MessageType.text)
                    }
                    break
/****************************
*MENU STALK******************************/
...
//------------------< Math Random >-------------------
		  case 'ganteng':
					...
					let kmn = `Cowok paling ganteng di group ini adalah\n*@${aku.jid.split('@')[0]}*`
					mentions(kmn, [aku.jid, cintax.jid], true)
					break
		 case 'cantik':
					if (!isGroup)return reply(mess.only.group)
					var kamu = groupMembers
          ...
          case 'resetlink': case 'revokelink': case 'revoke':
            ...
            case 'welcome':
                 ...
                 case 'antilink':
                       ...
                       antilink.push(from)
					   fs.writeFileSync('./lin/antilink.json', JSON.stringify(antilink))
